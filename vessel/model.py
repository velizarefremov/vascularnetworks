...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = tf.reshape(features["x"], [-1, 256, 256, 256, 1])
...
def load_train_data(filename, type=0):
    ...
    if type == 0:
        data = np.zeros([1, 256, 256, 256], dtype=np.float32)
    else:
        data = np.zeros([1, 256, 256, 256], dtype=np.int32)
...
def load_test_data():
    ...
    data[0] = np.array(get_itk_array('..\\input\\vessel\\test.nii.gz'))
